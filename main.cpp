...
int main (){
	...
	cl_context context = clCreateContextFromType (
		( NULL == platform ) ? NULL : properties, CL_DEVICE_TYPE_GPU, NULL, NULL, &error );
	...
	error = clEnqueueWriteBuffer (queue, x, CL_TRUE, 0, sizeof(float) * SIZE, data_x, 0, NULL, NULL);
	...
	clEnqueueReadBuffer (queue, y, CL_TRUE, 0, sizeof(float) * n, results, 0, NULL, NULL);
	...
	cl_context_properties properties_d [3] = { CL_CONTEXT_PLATFORM, ( cl_context_properties ) platform, 0 };

	cl_context context_d = clCreateContextFromType (( NULL == platform ) ? NULL : properties_d, CL_DEVICE_TYPE_GPU, NULL, NULL, &error);
	...
	clGetContextInfo (context_d, CL_CONTEXT_DEVICES, 0,	NULL, &size );
	...
	{
		cl_device_id * devices = ( cl_device_id * ) alloca ( size );
		clGetContextInfo (context_d, CL_CONTEXT_DEVICES, size, devices, NULL );
		device_d = devices[0];
	...
	cl_command_queue queue_d = clCreateCommandQueue(context_d, device_d, CL_QUEUE_PROFILING_ENABLE, &error );
	...
	cl_program program_d = clCreateProgramWithSource(context_d, 1, &daxpy_gpu, srclen_d, &error );

	if (error != CL_SUCCESS) {
		std::cout << "Create program failed: " << error << std::endl;
	}

	error = clBuildProgram(program_d, 1, &device_d, NULL, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Build program failed: " << error << std::endl;
	}

	cl_kernel kernel_d = clCreateKernel(program_d, "da_gpu", &error);
	...
	cl_mem x_d = clCreateBuffer (context_d, CL_MEM_READ_ONLY, sizeof(double) * SIZE, NULL, NULL);

	cl_mem y_d = clCreateBuffer (context_d, CL_MEM_READ_WRITE, sizeof(double) * SIZE, NULL,	NULL);

	error = clEnqueueWriteBuffer (queue_d, x_d, CL_TRUE, 0, sizeof(double) * SIZE, data_x_d, 0, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Enqueue write buffer data_x failed: " << error << std::endl;
	}

	error = clEnqueueWriteBuffer (queue_d, y_d, CL_TRUE, 0,	sizeof(double) * SIZE, data_y_d, 0,	NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Enqueue write buffer data_x failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_d, 0, sizeof(int), &n);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for n failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_d, 1, sizeof(double), &a_d);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for a failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_d, 2, sizeof(cl_mem), &x_d);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for x failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_d, 3, sizeof(int), &incx_d);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for incx failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_d, 4, sizeof(cl_mem), &y_d);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for y failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_d, 5, sizeof(int), &incy_d);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for incy failed: " << error << std::endl;
	}

	clGetKernelWorkGroupInfo (kernel_d, device_d, CL_KERNEL_WORK_GROUP_SIZE, sizeof (size_t), &group, NULL);

	cl_event evt1;
	auto start_dgpu = std::chrono::steady_clock::now();

	error = clEnqueueNDRangeKernel (queue_d, kernel_d, 1, NULL, &n, NULL, 0, NULL, &evt1 );
	...
	clEnqueueReadBuffer (queue_d, y_d, CL_TRUE, 0, sizeof(double) * n, results_d, 0, NULL, NULL);
	...
	cl_context context_fcpu = clCreateContextFromType (( NULL == platform ) ? NULL : properties, CL_DEVICE_TYPE_CPU, NULL, NULL, &error );

	if (error != CL_SUCCESS) {
		std::cout << "Create context from type failed: " << error << std::endl;
	}

	clGetContextInfo (context_fcpu, CL_CONTEXT_DEVICES, 0, NULL, &size );
	...
	{
		cl_device_id * devices = ( cl_device_id * ) alloca ( size );
		clGetContextInfo (context_fcpu, CL_CONTEXT_DEVICES, size, devices, NULL);
	...
	cl_program program_fcpu = clCreateProgramWithSource(context_fcpu, 1, &saxpy_gpu, srclen_fcpu, &error );

	if (error != CL_SUCCESS) {
		std::cout << "Create program failed: " << error << std::endl;
	}

	error = clBuildProgram(program_fcpu, 1, &device_fcpu, NULL, NULL, NULL);
	...
	cl_mem x_fcpu = clCreateBuffer (context_fcpu, CL_MEM_READ_ONLY, sizeof(float) * SIZE, NULL, NULL);

	cl_mem y_fcpu = clCreateBuffer (context_fcpu, CL_MEM_READ_WRITE, sizeof(float) * SIZE, NULL, NULL);

	error = clEnqueueWriteBuffer (queue_fcpu, x_fcpu, CL_TRUE, 0, sizeof(float) * SIZE, data_x_fcpu, 0, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Enqueue write buffer data_x failed: " << error << std::endl;
	}

	error = clEnqueueWriteBuffer (queue_fcpu, y_fcpu, CL_TRUE, 0, sizeof(float) * SIZE, data_y_fcpu, 0, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Enqueue write buffer data_x failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_fcpu, 0, sizeof(int), &n);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for n failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_fcpu, 1, sizeof(float), &a);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for a failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_fcpu, 2, sizeof(cl_mem), &x_fcpu);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for x failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_fcpu, 3, sizeof(int), &incx);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for incx failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_fcpu, 4, sizeof(cl_mem), &y_fcpu);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for y failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_fcpu, 5, sizeof(int), &incy);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for incy failed: " << error << std::endl;
	}

	clGetKernelWorkGroupInfo (kernel_fcpu, device_fcpu, CL_KERNEL_WORK_GROUP_SIZE, sizeof (size_t), &group, NULL );

	cl_event evt_fcpu;

	error = clEnqueueNDRangeKernel ( queue_fcpu, kernel_fcpu, 1, NULL, &n, &localSize_cpu, 0, NULL, &evt_fcpu );
	...
	clEnqueueReadBuffer (queue_fcpu, y_fcpu, CL_TRUE, 0, sizeof(float) * n, results_fcpu, 0, NULL, NULL);

	std::cout << "CPU OpenCL time for SAXPY = " << (cl_double)(end2 - start2)*(cl_double)(1e-06) << "ms" << std::endl;

	// ---------------------------------------- for CPU double ----------------------------------------

	cl_context context_dcpu = clCreateContextFromType (( NULL == platform ) ? NULL : properties, CL_DEVICE_TYPE_CPU, NULL, NULL, &error );

	if (error != CL_SUCCESS) {
		std::cout << "Create context from type failed: " << error << std::endl;
	}

	clGetContextInfo (context_dcpu, CL_CONTEXT_DEVICES, 0, NULL, &size );
	...
	cl_command_queue queue_dcpu = clCreateCommandQueue(context_dcpu, device_dcpu, CL_QUEUE_PROFILING_ENABLE, &error );
	...
	cl_program program_dcpu = clCreateProgramWithSource(context_dcpu, 1, &daxpy_gpu, srclen_dcpu, &error );

	if (error != CL_SUCCESS) {
		std::cout << "Create program failed: " << error << std::endl;
	}

	error = clBuildProgram(program_dcpu, 1, &device_dcpu, NULL, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Build program failed: " << error << std::endl;
	}

	cl_kernel kernel_dcpu = clCreateKernel(program_dcpu, "da_gpu", &error);
	...
	cl_mem x_dcpu = clCreateBuffer (context_dcpu, CL_MEM_READ_ONLY, sizeof(double) * SIZE, NULL, NULL);

	cl_mem y_dcpu = clCreateBuffer (context_dcpu, CL_MEM_READ_WRITE, sizeof(double) * SIZE, NULL, NULL);

	error = clEnqueueWriteBuffer (queue_dcpu, x_dcpu, CL_TRUE, 0, sizeof(double) * SIZE, data_x_dcpu, 0, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Enqueue write buffer data_x failed: " << error << std::endl;
	}

	error = clEnqueueWriteBuffer (queue_dcpu, y_dcpu, CL_TRUE, 0, sizeof(double) * SIZE, data_y_dcpu, 0, NULL, NULL);

	if (error != CL_SUCCESS) {
		std::cout << "Enqueue write buffer data_x failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_dcpu, 0, sizeof(int), &n);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for n failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_dcpu, 1, sizeof(double), &a_d);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for a failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_dcpu, 2, sizeof(cl_mem), &x_dcpu);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for x failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_dcpu, 3, sizeof(int), &incx);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for incx failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_dcpu, 4, sizeof(cl_mem), &y_dcpu);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for y failed: " << error << std::endl;
	}

	error = clSetKernelArg (kernel_dcpu, 5, sizeof(int), &incy);

	if (error != CL_SUCCESS) {
		std::cout << "Set kernel args for incy failed: " << error << std::endl;
	}

	clGetKernelWorkGroupInfo (kernel_dcpu, device_dcpu, CL_KERNEL_WORK_GROUP_SIZE, sizeof (size_t), &group,NULL );

	cl_event evt_dcpu;

	error = clEnqueueNDRangeKernel (queue_dcpu, kernel_dcpu, 1, NULL, &n, NULL, 0, NULL, &evt_dcpu );
	...
	clEnqueueReadBuffer (queue_dcpu, y_dcpu, CL_TRUE, 0, sizeof(double) * n, results_dcpu, 0, NULL, NULL);
